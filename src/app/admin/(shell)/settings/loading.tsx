...
export default function SettingsLoading(): React.ReactElement {
  return (
    <div role="status" aria-live="polite" aria-label="Loading settings page">
      <Bone className="mb-1 h-8 w-36 bg-slate-200" />
      <Bone className="mb-6 h-4 w-full max-w-xl bg-slate-200 opacity-60" />

      {/* Tab strip */}
      <div className="mb-6 flex gap-1 overflow-x-auto border-b border-slate-200 pb-px">
        {Array.from({ length: 8 }).map((_, i) => (
          <Bone key={i} className="h-9 w-24 shrink-0 bg-slate-200" />
        ))}
      </div>
        ...
        <div className="flex flex-col gap-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="flex flex-col gap-2">
              ...
              <Bone className="h-3 w-full max-w-lg bg-slate-200 opacity-60" />
              <Bone className="h-10 w-full max-w-xs bg-slate-200" />
            </div>
          ))}
          <div className="flex gap-3 border-t border-slate-100 pt-4">
            <Bone className="h-10 w-28 bg-slate-200" />
            <Bone className="h-10 w-24 bg-slate-200" />
          </div>
        </div>
      </div>
    </div>
