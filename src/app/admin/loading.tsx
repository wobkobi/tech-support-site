  ...
  return (
    <AdminSkeletonShell label="Loading dashboard">
      ...
      <div className={cn("mb-6 flex flex-wrap gap-3")}>
        <Bone className={cn("h-10 w-40 bg-slate-200")} />
        <Bone className={cn("h-10 w-40 bg-slate-200")} />
      </div>
      ...
      <div className={cn("mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4")}>
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className={cn("rounded-xl border border-slate-200 bg-white px-4 py-4 shadow-sm")}
          >
            <Bone className={cn("mb-2 h-7 w-20 bg-slate-200")} />
      ...
      <div className={cn("grid grid-cols-1 gap-6 lg:grid-cols-2")}>
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className={cn("rounded-xl border border-slate-200 bg-white shadow-sm")}>
            <div
              className={cn(
            ...
            <div className={cn("divide-y divide-slate-100")}>
              {Array.from({ length: 4 }).map((_, j) => (
                <div key={j} className={cn("flex items-start justify-between gap-3 px-5 py-3")}>
                  <div className={cn("min-w-0 flex-1")}>
                    <Bone className={cn("mb-1.5 h-4 w-32 max-w-full bg-slate-200")} />
                    <Bone className={cn("h-3 w-40 max-w-full bg-slate-200 opacity-60")} />
                  </div>
                  <Bone className={cn("h-3 w-16 shrink-0 bg-slate-200 opacity-60")} />
                </div>
              ))}
            </div>
          </div>
