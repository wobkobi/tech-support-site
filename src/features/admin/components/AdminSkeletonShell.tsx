...
export function AdminSkeletonShell({
  ...
  return (
    ...
    >
      ...
      <aside
        className="hidden h-screen w-56 shrink-0 flex-col bg-russian-violet px-3 py-4 lg:flex"
        aria-hidden="true"
      >
        <div className="border-b border-white/10 px-2 pb-4">
          <Bone className="mb-1 h-3 w-12 bg-white/20" />
          <Bone className="h-4 w-28 bg-white/20" />
        </div>
        <div className="flex flex-col gap-1 pt-4">
          {Array.from({ length: 7 }).map((_, i) => (
            <Bone key={i} className="h-9 w-full bg-white/10" />
          ))}
        </div>
      </aside>
