...
- destination + noTravelCharge: ONE decision, worked through in this order. A job bills a single round trip, never two.
  STEP 1 - Did the job cause ANY trip by car, vehicle, or public transport? A drive to collect parts that appear in parts[] on this invoice counts. A supply run whose items do NOT appear in parts[] does NOT count (restocking or a warranty errand is not this client's trip). If no such trip happened - everything on foot or by bicycle, remote, or worked from home - set destination null and noTravelCharge true, and STOP HERE.
  STEP 2 - Was the CLIENT's place reached by vehicle? Then destination is the client's suburb or address (e.g. "Papakura", "123 Smith St Manukau"). A supply stop on the same outing is a detour on the way: ignore it completely, no second destination and no warning.
  STEP 3 - Otherwise the supply run was the only vehicle trip: destination is that place.
  STEP 4 - Anything reaching step 2 or 3 has noTravelCharge false.
  INVARIANT - destination and noTravelCharge always agree. A non-null destination ALWAYS pairs with noTravelCharge false; noTravelCharge true ALWAYS pairs with a null destination. NEVER name a destination and then refuse to charge it. Walking to the client while driving a billed supply run is a CHARGED trip to that supply run - it is not a free walk, and it does not get the on-foot warning.
...
- statedDistanceKm: If the user explicitly states a travel distance AND traveled by car/vehicle OR public transport, set this to the total round-trip km as a number (e.g. "traveled 10 km there and back" → 10, "drove 8 km each way" → 16). Set to null if no distance was stated, or if travel was by foot or bicycle.
  - Walking or cycling: set statedDistanceKm to null. Whether anything is charged was already settled by the destination decision above - only add the warning "Traveled by [mode] - no travel charge applied (assumed local job). Verify if this should be charged." when that decision stopped at STEP 1. If it reached step 3, the drive bills and this warning is wrong.
...
- noTravelCharge: set by the destination decision above - true only when it stopped at STEP 1. Never decide it separately from destination.
