...
- destination + noTravelCharge: ONE decision, worked through in this order. A job bills a single round trip, never two.
  ...
  STEP 2 - Did the job cause ANY trip by car, vehicle, or public transport? A drive made only to collect parts counts as one. Answer for the WHOLE description before moving on: a walk to the client does NOT settle this when a drive appears in a later sentence, so read to the end first.
    - YES, at least one vehicle trip: go to STEP 3.
    - NO, every trip named was on foot or by bicycle, or the work was explicitly remote or done at home: destination null, noTravelCharge TRUE, STOP. Only here does the operator actually say the trip is not chargeable.
  STEP 3 - Was the CLIENT's place reached by vehicle? Then destination is the client's suburb or address (e.g. "Papakura", "123 Smith St Manukau"). A supply stop on the same outing is a detour on the way: ignore it completely, no second destination and no warning.
  STEP 4 - The client was NOT reached by vehicle, so the only vehicle trip was a supply run. Did what it collected end up in parts[] on THIS invoice?
    - YES: destination is that supply stop. The job caused that drive, so the job pays for it.
    - NO: restocking, a warranty return, or an errand of the operator's own - not this client's trip. Answer as STEP 1 does (destination null, noTravelCharge false) and STOP. Never bill a client for a drive that put nothing on their invoice.
  STEP 5 - Anything reaching step 3 or 4 has noTravelCharge false.
...
- statedDistanceKm: If the user explicitly states a travel distance AND travelled by car/vehicle OR public transport, set this to the total round-trip km as a number (e.g. "travelled 10 km there and back" → 10, "drove 8 km each way" → 16). Set to null if no distance was stated, or if travel was by foot or bicycle.
  - Walking or cycling: set statedDistanceKm to null. Whether anything is charged was already settled by the destination decision above - only add the warning "Travelled by [mode] - no travel charge applied (assumed local job). Verify if this should be charged." when that decision stopped at STEP 2. If it reached step 4, the drive bills and this warning is wrong.
...
- noTravelCharge: set by the destination decision above - true only when it stopped at STEP 2. Never decide it separately from destination, and never set it true just because travel went unmentioned.
