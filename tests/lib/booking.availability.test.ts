...
describe("buildAvailableDays & validateBookingRequest", () => {
  ...
  describe("date label and dateKey formatting", () => {
    it("produces the correct NZ dateKey when UTC and NZ are on different calendar days", () => {
      // 2026-01-11T11:00:00.000Z = midnight 2026-01-12 in NZDT (UTC+13).
      // January is solidly within NZDT (NZ DST runs last Sun Sept – first Sun Apr).
      // Old bug: setHours(0,0,0,0) used local timezone → toISOString() gave "2026-01-11" (Sunday).
      // Fixed:  toLocaleDateString("en-CA") gives "2026-01-12" (Monday).
      vi.setSystemTime(new Date("2026-01-11T11:00:00.000Z"));
      const now = new Date();
    ...
    it("dayLabel contains the correct NZ day name, not the UTC day name", () => {
      // Same midnight-NZ scenario: NZ day is Monday 12 Jan, UTC day is Sunday 11 Jan.
      vi.setSystemTime(new Date("2026-01-11T11:00:00.000Z"));
      const now = new Date();

      const days = buildAvailableDays([], [], now, BOOKING_CONFIG);

      // dayLabel format is "Mon 12 Jan"
      expect(days[0].dayLabel).toBe("Mon 12 Jan");
    });
    ...
    it("fullLabel contains the correct NZ day name", () => {
      ...
      // fullLabel format is "Monday, Jan 12"
      expect(days[0].fullLabel).toBe("Monday, Jan 12");
    });

    it("isWeekend is false for a Monday at NZ midnight (not Sunday)", () => {
      vi.setSystemTime(new Date("2026-01-11T11:00:00.000Z")); // NZ midnight Monday 12 Jan
      const now = new Date();
    ...
    it("dateKey rolls over correctly at NZ midnight across a month boundary", () => {
      // 2026-01-31T11:00:00.000Z = midnight 2026-02-01 NZDT — end of Jan to start of Feb
      vi.setSystemTime(new Date("2026-01-31T11:00:00.000Z"));
      const now = new Date();
    ...
    it("dateKey and labels are correct mid-afternoon NZ (both UTC and NZ same calendar day)", () => {
      // 2026-01-12T02:00:00.000Z = 3pm Monday 12 Jan NZDT — no date boundary difference
      vi.setSystemTime(new Date("2026-01-12T02:00:00.000Z"));
      const now = new Date();
  ...
  describe("DST timezone handling", () => {
    it("correctly handles NZDT (UTC+13) during summer", () => {
      // Feb 24, 2026 is solidly within NZDT (UTC+13; DST runs last Sun Sept – first Sun Apr)
      vi.setSystemTime(new Date("2026-02-24T10:00:00.000Z"));
      const now = new Date();
